...
$DeployedPackages = Get-CMPackageDeployment

$TotalDeployedPackages = $DeployedPackages.count

$i = 0

Foreach ($DeployedPackage in $DeployedPackages)
{
    $PackageInfo = Get-CMPackage -Fast -Id $DeployedPackage.PackageID
    $i++
    $percentComplete = ($i / $TotalDeployedPackages) * 100
    Write-Progress -Activity "Item $i of $TotalDeployedPackages" -PercentComplete $percentComplete -Status "$(([math]::Round((($i)/$TotalDeployedPackages * 100),0))) % complete" -CurrentOperation "Enabling `"On-demand distribution`" for `"$($PackageInfo.Name)`""
    ...
    Set-CMPackage -Id $DeployedPackage.PackageID -SendToPreferredDistributionPoint $true
}
